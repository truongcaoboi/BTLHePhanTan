...
public class Security {
	...
	private PublicKey KeyPublicOfServer=null;
	private PublicKey KeyPublicOfClient=null;
	...
	public void SetPublicKeyOfServer(String s) {
		try {
			byte[] keys = s.getBytes();
			X509EncodedKeySpec spec= new X509EncodedKeySpec(keys);
			KeyFactory kf=KeyFactory.getInstance("RSA");
			KeyPublicOfServer=kf.generatePublic(spec);
		}catch(Exception e) {
			e.printStackTrace();
		}
	}
	
	public void SetPublicKeyOfClient(String s) {
		try {
			byte[] keys = s.getBytes();
			X509EncodedKeySpec spec= new X509EncodedKeySpec(keys);
			KeyFactory kf=KeyFactory.getInstance("RSA");
			KeyPublicOfClient=kf.generatePublic(spec);
		}catch(Exception e) {
			e.printStackTrace();
		}
	}
	
	public String GetPublicKeyOfServer() {
		return KeyPublicOfServer.toString();
	}
...
//		PublicKey pubKey=factory.generatePublic(spec);
		try {
			...
			c.init(Cipher.ENCRYPT_MODE, KeyPublicOfServer);
	...
	private String EncryptToClient(String input) {
//		X509EncodedKeySpec spec= new X509EncodedKeySpec(KeyPublic.getEncoded());
//		KeyFactory factory=KeyFactory.getInstance("RSA");
//		PublicKey pubKey=factory.generatePublic(spec);
		try {
			Cipher c=Cipher.getInstance("RSA");
			c.init(Cipher.ENCRYPT_MODE, KeyPublicOfClient);
			String strEncode=new String(Base64.getEncoder().encodeToString(c.doFinal(input.getBytes()))).toString();
			return strEncode;
		} catch (Exception e) {
	...
	public String MaHoa1(String input) {
		return EncryptToClient(input);
	}
}
