...
public class Display {
	...
	public static DefaultTableModel showTransaction(Vector<Transaction> tran) {
		...
		String [] column= {"Mã hóa đơn", "Người thực hiện","Số tài khoản", "Người nhận","Số tài khoản", "Loại giao dịch", "Ngày giao dịch", "Số tiền", "Địa điểm giao dịch", "Trạng thái"};
		DefaultTableModel model=new DefaultTableModel(new Object[][] {},column);
		for(Transaction t:tran) {
			String [] row= {t.getBill_no(),t.getNameFrom(),t.getAccount_no(),t.getNameTo(),t.getReceiver(),t.getType(),t.getDateCreate(),t.getAmount()+"",t.getExchange_at(),t.getSta()};
			model.addRow(row);
		}
		return model;
	}
